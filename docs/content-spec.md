...
- `/process.html`：原始需求、訪談轉折、完整製作流程、問題修正、使用工具與可追溯 Git 紀錄。
...
- 驗證建置來源路由 `/process`、`/lab`、`/tempo`，以及公開路徑 `/process.html`、`/lab.html`、`/tempo.html`。
...
- `/`、`/process.html`、`/lab.html`、`/tempo.html` 在 Hugging Face Static Space 可直接開啟，且站內不得連到會回傳 `Entry not found` 的無副檔名路徑。
